...
@dataclass
class SpladeModelArguments(ModelArguments):
    stopwords_weight: float = field(default=None)
    latent_vocab_size: int = field(default=None)
    mask_file: str = field(default=None)

    # def __post_init__(self):
    #     if self.stopwords_weight and self.latent_vocab_size is not None:
    #         raise ValueError('--stopwords_only and --laten_vocab_size cannot be set at the same time.')
